    ...
    <div class="IconTypeList" v-if="$store.state.showType == 'icon'">
      ...
      >
        <div class="imgContainer">
          ...
          />
          <!-- 未知文件 -->
          <img src="~assets/img/unknown.png" alt="" :draggable="false" v-else />
          <img
  ...
  methods: {
    ...
    selectAll(flag) {
      ...
      if (flag) {
        ...
        //   // 文件夹不选中
        //   if (index < this.currentChildrenFolder.length) return;
        //   listItem[index].classList.add("selectFile");
        //   this.selectFiles.push(index);
        // });
      } else {
        // this.selectFiles.forEach((item) => {
        //   listItem[item].classList.remove("selectFile");
        // });
        this.selectFiles = [];
      }
      this.getIsAllFileCollect();
    ...
    downloadCurrentFile(type, item) {
      ...
      arr.forEach((i) => {
        ...
        // setTimeout(() => {
        //   if (
        //     this.listData[item].filetype == "video" ||
        //     this.listData[item].filetype == "audio"
        //   ) {
        //     url = "/downloadvideo/" + this.listData[item].url.split("com/")[1];
        //   } else {
        //     url = "/downloadfile/" + this.listData[item].url.split("com/")[1];
        //   }
        //   this.$store.commit("updateCurrentDownloadFileInfo", {
        //     name: this.listData[item].name + "." + this.listData[item].type,
        //     url,
        //   });
        // });
        // 循环执行的速度太快，watch来不及监听 这里通过定时器放到异步执行
    ...
    onDragItemStart(e, item) {
      ...
      if (item.filetype != "video") {
        img1 = e.path[0].querySelector("img").cloneNode();
      } else {
        img1 = e.path[0].querySelector("video").cloneNode();
      }
        ...
        this.selectFiles.some((i) => {
          ...
          if (i.id != item.id) {
            ...
            if (i.filetype != "video") {
              ...
              // word
              else if (i.type == "doc" || i.type == "docx") {
                img.setAttribute("src", require("/src/assets/img/word.png"));
              }
              // excel
              else if (i.type == "xlsx" || i.type == "xls") {
                img.setAttribute("src", require("/src/assets/img/excel.png"));
              }
              // txt
              else if (i.type == "txt") {
                img.setAttribute("src", require("/src/assets/img/txt.png"));
              }
              // pdf
              else if (i.type == "pdf") {
                img.setAttribute("src", require("/src/assets/img/pdf.png"));
              }
              // zip
              else if (
                i.type == "zip" ||
                ...
                img.setAttribute("src", require("/src/assets/img/zip.png"));
              }
              // ppt
              else if (i.type == "ppt" || i.type == "pptx") {
                img.setAttribute("src", require("/src/assets/img/ppt.png"));
              }
              // unknown
              else {
                img.setAttribute("src", require("/src/assets/img/unknown.png"));
              }
      ...
      // console.log(dragImgContainer);

      // let img = e.path[0].querySelector("img");
